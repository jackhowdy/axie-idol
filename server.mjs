#!/usr/bin/env node
/**
 * Production server: dist/ + Axie metadata/image proxies + Idol feed API.
 */
...
const NOTIFS_FILE = resolve(DATA_DIR, 'notifications.json')
const OWNERS_FILE = resolve(DATA_DIR, 'owners.json')
const BURNS_FILE = resolve(DATA_DIR, 'burns.json')
const CAST_CREW_FILE = resolve(DATA_DIR, 'castCrew.json')
...
const SEED_ENABLED = process.env.SEED_POSTS === '1' || process.env.SEED_POSTS === 'true'
const SEED_CAPTIONS = {
 ...
 */
function seedCompositeImagePath(seedId, castId) {
  const name = `${seedId}.png`
  const full = join(UPLOADS_DIR, name)
  if (existsSync(full)) return `/uploads/${name}`
...
function loadCastCrew() {
  try {
    if (!existsSync(CAST_CREW_FILE)) {
      const empty = emptyCastCrew()
      writeFileSync(CAST_CREW_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    const raw = readFileSync(CAST_CREW_FILE, 'utf8')
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object') return emptyCastCrew()
    if (!data.byPoster || typeof data.byPoster !== 'object') data.byPoster = {}
    return data
  } catch (err) {
    console.warn('[server] castCrew.json load failed — resetting', err)
    return emptyCastCrew()
  }
}

function saveCastCrew(store) {
  const tmp = CAST_CREW_FILE + '.tmp'
  writeFileSync(tmp, JSON.stringify(store, null, 2))
  renameSync(tmp, CAST_CREW_FILE)
}
  ...
  try {
    const result = await execFileAsync(
      ...
      [
        ...
        '-H',
        'User-Agent: AxieIdolComposer/1.0 (+curl)',
        '--data-binary',
        `@${tmpPath}`,
        '--max-time',
        '25',
    ...
    const msg = err?.stderr || err?.message || 'GraphQL curl failed'
    throw Object.assign(new Error(String(msg).trim() || 'GraphQL curl failed'), {
      statusCode: 502,
    })
  } finally {
    try {
...
function loadStore() {
  try {
    if (!existsSync(POSTS_FILE)) {
      ...
      writeFileSync(POSTS_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    ...
    console.warn('[server] posts.json load failed — resetting', err)
    return emptyStore()
  }
}
...
function loadFollows() {
  try {
    if (!existsSync(FOLLOWS_FILE)) {
      const empty = emptyFollows()
      writeFileSync(FOLLOWS_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    const raw = readFileSync(FOLLOWS_FILE, 'utf8')
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object') return emptyFollows()
    ...
    console.warn('[server] follows.json load failed — resetting', err)
    return emptyFollows()
  }
}

function saveFollows(store) {
  const tmp = FOLLOWS_FILE + '.tmp'
  writeFileSync(tmp, JSON.stringify(store, null, 2))
  renameSync(tmp, FOLLOWS_FILE)
}
...
function loadNotifs() {
  try {
    if (!existsSync(NOTIFS_FILE)) {
      const empty = emptyNotifs()
      writeFileSync(NOTIFS_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    const raw = readFileSync(NOTIFS_FILE, 'utf8')
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object') return emptyNotifs()
    if (!data.byAddress || typeof data.byAddress !== 'object') data.byAddress = {}
    for (const [k, v] of Object.entries(data.byAddress)) {
      if (!Array.isArray(v)) data.byAddress[k] = []
    }
    ...
    console.warn('[server] notifications.json load failed — resetting', err)
    return emptyNotifs()
  }
}

function saveNotifs(store) {
  const tmp = NOTIFS_FILE + '.tmp'
  writeFileSync(tmp, JSON.stringify(store, null, 2))
  renameSync(tmp, NOTIFS_FILE)
}
...
function loadOwners() {
  try {
    if (!existsSync(OWNERS_FILE)) {
      const empty = emptyOwners()
      writeFileSync(OWNERS_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    const raw = readFileSync(OWNERS_FILE, 'utf8')
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object' || Array.isArray(data)) return emptyOwners()
    return data
  } catch (err) {
    console.warn('[server] owners.json load failed — resetting', err)
    return emptyOwners()
  }
}
...
function loadBurns() {
  try {
    if (!existsSync(BURNS_FILE)) {
      const empty = emptyBurns()
      writeFileSync(BURNS_FILE, JSON.stringify(empty, null, 2))
      return empty
    }
    const raw = readFileSync(BURNS_FILE, 'utf8')
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object') return emptyBurns()
    if (!Array.isArray(data.settledDays)) data.settledDays = []
    if (!Array.isArray(data.records)) data.records = []
    data.mode = BURN_MODE
    return data
  } catch (err) {
    console.warn('[server] burns.json load failed — resetting', err)
    return emptyBurns()
  }
}

function saveBurns(store) {
  const tmp = BURNS_FILE + '.tmp'
  writeFileSync(tmp, JSON.stringify(store, null, 2))
  renameSync(tmp, BURNS_FILE)
}
...
async function fetchWaypointProfile(idToken) {
  const payload = JSON.stringify({ id_token: idToken })
  const tmpPath = join(
    tmpdir(),
    `axie-idol-wp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  )
  ...
  try {
    ...
    const status = Number(String(result.stdout || '').trim()) || 0
    let json = {}
    try {
...
function safeJoinDist(urlPath) {
  ...
  const full = resolve(DIST, '.' + (cleaned.startsWith('/') ? cleaned : '/' + cleaned))
  if (!full.startsWith(DIST)) return null
  return full
}

function safeJoinUploads(urlPath) {
  // /uploads/xxx.jpg → UPLOADS_DIR/xxx.jpg
  const name = urlPath.replace(/^\/uploads\/?/, '').split('?')[0]
...
function serveUpload(pathname, res) {
  const filePath = safeJoinUploads(pathname)
...
async function handleCreatePost(req, res) {
  ...
  writeFileSync(diskPath, parsed.buf)
...
if (!existsSync(DIST)) {
  console.error('[server] Missing dist/')
  process.exit(1)
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // CORS-ish for same-origin + tunnel
...
// Boot-time empty-room seed (no-op when live posts are plentiful)
try {
  const bootStore = loadStore()
  ensureSeedPosts(bootStore)
} catch (err) {
  console.warn('[axie-idol] seed boot skipped', err?.message || err)
}

server.listen(PORT, HOST, () => {
  console.log(`[axie-idol] serving ${DIST} on http://${HOST}:${PORT}`)
  console.log(`[axie-idol] feed store ${POSTS_FILE}`)
  console.log(`[axie-idol] burns ledger ${BURNS_FILE} (mode=${BURN_MODE}, pot=${DAILY_POT_SLP}, spark=${SPARK_SLP})`)
  console.log(`[axie-idol] cast crew ${CAST_CREW_FILE}`)
})
