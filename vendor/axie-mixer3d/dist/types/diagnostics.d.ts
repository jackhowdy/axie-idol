import type { AxiePartAssetId, AxieRigType } from './domain.js';
import type { AxieAnimationSet, AxieShaderFidelity } from './manifest.js';
import type { AxieQualityId } from './quality.js';
export type AxieDiagnosticSeverity = 'info' | 'warning' | 'error';
export type AxieDiagnosticCode = 'manifest-invalid' | 'source-inventory-mismatch' | 'genes-invalid' | 'body-missing' | 'part-missing' | 'attach-node-missing' | 'lod-clamped' | 'texture-variant-fallback' | 'texture-missing' | 'shader-missing' | 'shader-fallback' | 'material-missing' | 'addon-missing' | 'animation-missing' | 'animation-track-unbound' | 'weapon-missing' | 'weapon-anchor-missing' | 'weapon-basis-missing' | 'weapon-load-failed' | 'load-aborted' | 'webgl-context-lost';
...
export interface AxieCoverageReport {
    ...
    readonly bodyCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
    readonly partDescriptorCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
    readonly rigCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
    readonly lodCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
    readonly materialCoverage: Readonly<{
        expected: number;
        ported: number;
    }>;
    readonly shaderCoverage: Readonly<{
        expected: number;
        ported: number;
    }>;
    readonly addonCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
    readonly animationCoverage: Readonly<{
        expected: number;
        exported: number;
    }>;
