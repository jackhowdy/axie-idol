...
- Genes do not carry part stage and the mixer decoder emits every part at stage 1, so `/api/axie/:id` also returns the marketplace stage per slot and the client applies it (all Nightmare and Nightmare-shiny parts are stage 2). Nightmare-body Axies use the pack's white `<class>-nightmare` palette on the normal body, as the mixer's own showcase does; the pack has no Nightmare body mesh, so `addNightmareSpikes` adds nine class-coloured toon thorns on an ellipsoid fitted from the part joints, parented to the spine bone so they follow the idle animation.
