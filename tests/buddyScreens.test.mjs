...
test('stardom on screen: the badge, the road to Idol, the title card, the Hall of Idols', () => {
  const rising = { days: 3, streak: 3, best: 3, title: { id: 'rising', name: 'Rising Star' }, shining: true, joyBonus: 4, fallen: null, next: { id: 'star', name: 'Star', needStreak: 7, haveStreak: 3 } }
  assert.match(starBadgeHtml(rising), /bd-star bd-star-rising shining/); assert.match(starBadgeHtml(rising), /Rising Star/); assert.match(starBadgeHtml(rising), /Miss a day and it goes/)
  assert.equal(starBadgeHtml({ days: 9, streak: 0, best: 7, title: { id: 'newcomer', name: 'Newcomer' }, shining: false, fallen: 'Star', next: null }), '', 'a lapsed streak holds no badge')
  ...
  assert.match(line, /Miss a day and Rising Star goes/); assert.doesNotMatch(line, /in all|for life/)
  const fallen = starProgressHtml({ days: 9, streak: 1, best: 14, title: { id: 'newcomer', name: 'Newcomer' }, shining: false, fallen: 'Idol', next: { id: 'rising', name: 'Rising Star', needStreak: 3, haveStreak: 1 } })
  assert.match(fallen, /It was an Idol\. Win it back\./, 'what there is to win back, never whose fault it was')
  assert.match(starProgressHtml({ days: 40, streak: 16, best: 16, title: { id: 'idol', name: 'Idol' }, shining: true, fallen: null, next: null }), /Idol, 16 joy days in a row.*Miss a day and the crown passes on/s)
  ...
  assert.match(home, /bd-star-rising shining/); assert.match(home, /joy days in a row to <b>Star/); assert.match(home, /3 joy days in a row/)
  assert.match(homeHtml({ ...miso, joy: { days: 30, streak: 14, best: 14, title: { id: 'idol', name: 'Idol' }, shining: true, fallen: null, next: null } }, null), /<h1 class="bd-gold">Miso/, 'a reigning Idol has its name in gold')
  assert.doesNotMatch(homeHtml({ ...miso, joy: { days: 30, streak: 0, best: 14, title: { id: 'newcomer', name: 'Newcomer' }, shining: false, fallen: 'Idol', next: { id: 'rising', name: 'Rising Star', needStreak: 3, haveStreak: 0 } } }, null), /<h1 class="bd-gold">/, 'and loses the gold with the streak')
  const card = joyHtml({ joyBonus: 4, joyStreak: 3, joy: rising, newTitle: { id: 'rising', name: 'Rising Star', line: 'I feel shiny.' } }, miso)
  assert.match(card, /Miso is a Rising Star/); assert.match(card, /I feel shiny\./); assert.match(card, /worth \+4 bond/); assert.match(card, /as long as the streak lasts/); assert.match(card, /data-action="sheet-next"/)
  assert.match(joyHtml({ joyBonus: 6, joyStreak: 14, joy: rising, newTitle: { id: 'idol', name: 'Idol', line: 'x' } }, miso), /Miso is an Idol/)
  const ladder = monthlyHtml({ month: '2026-09', endsAt: new Date(Date.now() + 5 * 864e5).toISOString(), you: null, rows: [{ rank: 1, buddyId: 'a', name: 'Nova', class: 'Bird', kind: 'visit', traits: [], level: 5, monthlyBond: 40, rarity: 0.5, title: 'idol', shining: true }], idols: [{ buddyId: 'a', name: 'Nova', class: 'Bird', kind: 'visit', axieId: '9', joyDays: 31, streak: 15, best: 15 }] })
  assert.match(ladder, /Hall of Idols/); assert.match(ladder, /reigning now/); assert.match(ladder, /<b class="bd-gold">Nova<\/b><small>15 joy days in a row/); assert.match(ladder, /bd-star bd-star-idol shining/)
  assert.match(monthlyHtml({ month: '2026-09', endsAt: new Date().toISOString(), you: null, rows: [] }), /only while the streak lasts/)
...
test('the front door: header with the mark, hero with a real photo and line, sections, honest rules; as About it only leads back', () => {
  ...
  assert.match(fresh, /id="lp-happy"/, 'and has its own section')
  ...
  assert.match(fresh, /Its class decides what makes it happiest/)
  ...
  assert.match(fresh, /Why does being an Idol matter\?/, 'the reason an Idol matters lives in the questions')
  // the front door is short on purpose: the idea, how it works, happiness, the roadmap, questions
  for (const gone of ['lp-why', 'lp-voice', 'lp-grow', 'lp-hatch']) assert.doesNotMatch(fresh, new RegExp(`id="${gone}"|href="#${gone}"`), gone)
  assert.doesNotMatch(fresh, /for life|for good|thirty in all|30 in all/, 'no title is permanent anywhere on the front door')
  ...
  for (const id of ['lp-prayer', 'lp-how', 'lp-happy', 'lp-road', 'lp-faq']) assert.match(fresh, new RegExp(`id="${id}"`), id)
  assert.match(fresh, /Is it free\?/)
  assert.match(fresh, /Ten a day build bond/)
  assert.equal((fresh.match(/data-action="meet"/g) || []).length, 3, 'header, hero and footer all lead to meeting an Axie')
  ...
  assert.match(fresh, /Pick a real Axie/)
